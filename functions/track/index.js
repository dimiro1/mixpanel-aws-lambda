...
exports.handle = function handle(event, context, callback) {
    ...
    if (event.callback) {
        callback(null,
            output(verbose,
                ERROR,
                'callback, jsonp not supported in this implementation'
            )
        );
        return;
    }
    ...
    if (event.img) {
        callback(null,
            output(verbose,
                ERROR,
                'img, img is not supported in this implementation'
            )
        );
        return;
    }
    ...
    try {
        data = JSON.parse(new Buffer(event.data, 'base64').toString('utf8'));
    } catch (e) {
    ...
    Firehose.putRecord(params, (err) => {
        if (err) {
            console.error(err);
            callback(null,
                output(verbose,
                    ERROR,
                    'firehose, could not send data to firehose'
                )
            );
        } else {
